...
class ImageEffects(image.Image):
	...
	def _pixelate(self, i, j, pixel_size, setter, *args, **kwargs):
		if j % pixel_size == 0 and i % pixel_size == 0:
			...
			if kwargs and 'jitter' in kwargs:
				jitter_x = random.randint(-kwargs['jitter'], kwargs['jitter'])
				jitter_y = random.randint(-kwargs['jitter'], kwargs['jitter'])
				args = args + (jitter_x,)
				args = args + (jitter_y,)
			for k in xrange(0, pixel_size**2):
	...
	def pointillism(self, radius, buffer=0, jitter=0, overwrite_img=True):
		...
		self.img = self.canvas if overwrite_img else self.img
